# 📖 AppBox ユーザーガイド

AppBoxはその名の通り「アプリを入れるための箱」であり、アプリを収集・管理するためのツールです。「コレクション」を作成してお気に入りのアプリを整理し、コレクションの説明、アプリの説明、カスタム属性情報の記録などの機能を提供します。

- [フィードバック](/ja/feedback/contact-developer)

## ❓ よくある質問

- [旧バージョンのデータ消失からの復元/復旧](/ja/faq/data-recovery)
- [アプリ地域とリンク認識](/ja/region/)
- [一部のアプリの詳細ページにスクリーンショットがない](/ja/faq/no-screenshots)
- [データ更新設定は何をするものですか？](/ja/faq/data-update-setting)
- [App Store からアプリをすばやくインポートする方法](/ja/faq/quick-import-appstore)

## 📥 アプリをお気に入りに追加する方法

- [AppBox 内で直接追加](/ja/how-to-add-apps/direct-add)
- [共有拡張機能でアプリを追加](/ja/how-to-add-apps/share-extension)
- [QRコードまたはリンクからインポート](/ja/how-to-add-apps/qr-link-import)

## 🔧 基本機能

### コレクション管理

- [コレクションの作成と編集](/ja/collection-management/create-edit)
- [コレクションビューの切り替え](/ja/collection-management/switch-view)
- [コレクションの並べ替え](/ja/collection-management/sort)
- [コレクションの共有](/ja/collection-management/share)
- [空のコレクションの検出](/ja/collection-management/empty-detect)

### アプリ管理

- [アプリ説明の編集](/ja/app-management/edit-description)
- [アプリビューの切り替え](/ja/app-management/switch-view)
- [アプリのコピー/移動](/ja/app-management/copy-move)
- [アプリの複数選択操作](/ja/app-management/multi-select)
- [コレクション内フィルター](/ja/app-management/filtering)

### 検索機能

- [アプリとコレクションの検索](/ja/search/)

### 統計機能

- [データ統計](/ja/statistics/)

### アプリタグ

- [タグ機能](/ja/tags/)

### アプリ詳細

- [アプリ詳細ページの説明](/ja/app-details/overview)
- [表示順序の並べ替え機能](/ja/app-details/sort-order)
- [翻訳機能](/ja/app-details/translation)

## ⭐ メンバーシップ

- [メンバーシップ機能の説明](/ja/membership/feature-overview)
- [購入に関する説明](/ja/membership/purchase)

## 🔄 データ同期

- [同期機能の説明](/ja/sync/overview)
- [同期が利用できない問題](/ja/sync/troubleshooting)

## 💾 データインポートとエクスポート

- [データバックアップ（データエクスポート）](/ja/backup-restore/export)
- [データインポート：AppBox からエクスポートしたデータ](/ja/backup-restore/data-import)
- [データインポート：Apple data](/ja/backup-restore/apple-data)

## ⚙️ 設定

- [テーマ設定（色とモード）](/ja/settings/theme)
- [表示設定](/ja/settings/display-configuration)
- [共有管理](/ja/settings/share-management)
- [アプリ属性](/ja/settings/app-properties)
- [データ管理](/ja/settings/data-management)

## 📄 その他

- [プライバシーポリシー](/ja/privacy-policy)
- [バージョンログ](/ja/roadmap/release-notes)
