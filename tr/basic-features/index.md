# 🔧 Temel Özellikler

AppBox'ın temel özellik modülleri, uygulamalarınızı daha iyi yönetmenize ve kullanmanıza yardımcı olur:

## 📁 Koleksiyon Yönetimi

- [Koleksiyon oluşturma ve düzenleme](/tr/collection-management/create-edit)
- [Koleksiyon görünümünü değiştirme](/tr/collection-management/switch-view)
- [Koleksiyon sıralama](/tr/collection-management/sort)
- [Koleksiyon paylaşma](/tr/collection-management/share)
- [Boş koleksiyon tespiti](/tr/collection-management/empty-detect)

## 📱 Uygulama Yönetimi

- [Uygulama açıklamasını düzenleme](/tr/app-management/edit-description)
- [Uygulama görünümünü değiştirme](/tr/app-management/switch-view)
- [Uygulama kopyalama/taşıma](/tr/app-management/copy-move)
- [Çoklu seçim işlemleri](/tr/app-management/multi-select)
- [Koleksiyon içi filtreleme](/tr/app-management/filtering)

## 🔍 Arama Özelliği

- [Uygulama ve Koleksiyon Arama](/tr/search/)

## 📊 İstatistik Özelliği

- [Veri İstatistikleri](/tr/statistics/)

## 🏷️ Uygulama Etiketleri

- [Etiket Özelliği](/tr/tags/)

## ℹ️ Uygulama Detayları

- [Uygulama detay sayfası açıklaması](/tr/app-details/overview)
- [Görüntüleme sırası sıralama](/tr/app-details/sort-order)
- [Çeviri özelliği](/tr/app-details/translation)
