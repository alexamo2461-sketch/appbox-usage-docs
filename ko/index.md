# 📖 AppBox 사용 가이드

AppBox는 이름 그대로 "앱을 담는 상자"로, 앱을 수집하고 관리하기 위한 도구입니다. "컬렉션"을 만들어 좋아하는 앱을 기록하고, 컬렉션 설명, 앱 설명, 사용자 정의 속성 정보 기록 등의 기능을 제공합니다.

- [피드백](/ko/feedback/contact-developer)

## ❓ 자주 묻는 질문

- [이전 버전 데이터 손실 복구/찾기](/ko/faq/data-recovery)
- [앱 지역 및 링크 인식](/ko/region/)
- [일부 앱의 상세 페이지에 스크린샷이 없습니다](/ko/faq/no-screenshots)
- [데이터 업데이트 설정은 무엇을 위한 것인가요?](/ko/faq/data-update-setting)
- [App Store에서 앱을 빠르게 가져오는 방법](/ko/faq/quick-import-appstore)

## 📥 앱을 즐겨찾기에 추가하는 방법

- [AppBox 내에서 직접 추가](/ko/how-to-add-apps/direct-add)
- [공유 확장 기능으로 앱 추가](/ko/how-to-add-apps/share-extension)
- [QR코드 또는 링크에서 가져오기](/ko/how-to-add-apps/qr-link-import)

## 🔧 기본 기능

### 컬렉션 관리

- [컬렉션 생성 및 편집](/ko/collection-management/create-edit)
- [컬렉션 보기 전환](/ko/collection-management/switch-view)
- [컬렉션 정렬](/ko/collection-management/sort)
- [컬렉션 공유](/ko/collection-management/share)
- [빈 컬렉션 감지](/ko/collection-management/empty-detect)

### 앱 관리

- [앱 설명 편집](/ko/app-management/edit-description)
- [앱 보기 전환](/ko/app-management/switch-view)
- [앱 복사/이동](/ko/app-management/copy-move)
- [앱 다중 선택 작업](/ko/app-management/multi-select)

### 앱 상세

- [앱 상세 페이지 설명](/ko/app-details/overview)
- [표시 순서 정렬 기능](/ko/app-details/sort-order)
- [번역 기능](/ko/app-details/translation)

## ⭐ 멤버십

- [멤버십 기능 설명](/ko/membership/feature-overview)
- [구매 관련 설명](/ko/membership/purchase)

## 🔄 데이터 동기화

- [동기화 기능 설명](/ko/sync/overview)
- [동기화 사용 불가 문제](/ko/sync/troubleshooting)

## 💾 데이터 가져오기 및 내보내기

- [데이터 백업 (데이터 내보내기)](/ko/backup-restore/export)
- [데이터 가져오기: AppBox에서 내보낸 데이터](/ko/backup-restore/data-import)
- [데이터 가져오기: Apple data](/ko/backup-restore/apple-data)

## ⚙️ 설정

- [데이터 업데이트](/ko/settings/data-update)
- [테마 설정 (색상 및 모드)](/ko/settings/theme)

## 📄 더보기

- [개인정보 처리방침](/ko/privacy-policy)
- [버전 로그](/ko/roadmap/release-notes)
